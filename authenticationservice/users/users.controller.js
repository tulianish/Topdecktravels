...
function validateQuestion(req, res, next) {
    userService.validateQuestion(req.body)
        .then(user => user ? res.json(user) : res.status(400).json({ message: 'Answer to the security question is incorrect' }))
        .catch(err => next(err));
}

function register(req, res, next) {
    userService.create(req.body)
        .then(() => res.status(400).json({ message: 'User registered successfully' }))
        .catch(err => next(err));
}
